...
// Expression represents a set of ExpressionTokens which, taken together,
// are an expression that can be evaluated down into a single value.
type Expression struct {

	// QueryDateFormat represents the query format used to output dates. Typically only used when creating SQL or Mongo queries from an expression.
	// Defaults to the complete ISO8601 format, including nanoseconds.
	QueryDateFormat string

	// ChecksTypes determines whether or not to safely check types when evaluating.
...
// NewExpression Parses a new Expression from the given [expression] string.
...
func NewExpression(expression string) (*Expression, error) {
	...
	return NewExpressionWithFunctions(expression, functions)
}

// NewExpressionFromTokens is similar to [NewExpression], except that instead of a string, an already-tokenized expression is given.
...
func NewExpressionFromTokens(tokens []ExpressionToken) (*Expression, error) {
	var ret *Expression
	var err error
...
// NewExpressionWithFunctions is similar to [NewExpression], except enables the use of user-defined functions.
// Functions passed into this will be available to the expression.
func NewExpressionWithFunctions(expression string, functions map[string]ExpressionFunction) (*Expression, error) {
	var ret *Expression
	var err error
...
// Evaluate is same as `Eval`, but automatically wraps a map of parameters into a `govalute.Parameters` structure.
func (expr Expression) Evaluate(parameters map[string]interface{}) (interface{}, error) {
	if parameters == nil {
...
func (expr Expression) Eval(parameters Parameters) (interface{}, error) {
	...
	if parameters != nil {
		...
		parameters = MapParameters(map[string]interface{}{})
	}

	return expr.evaluateStage(expr.evaluationStages, parameters)
}

func (expr Expression) evaluateStage(stage *evaluationStage, parameters Parameters) (interface{}, error) {
...
func (expr Expression) Tokens() []ExpressionToken {
	return expr.tokens
}

// String returns the original expression used to create this Expression.
func (expr Expression) String() string {
	return expr.inputExpression
}

// Vars returns an array representing the variables contained in this Expression.
func (expr Expression) Vars() []string {
	var varlist []string
